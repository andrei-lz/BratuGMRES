...
NewtonStats newton_solve(const Cart2D &cart, Grid &g, const Options &opt)
{
  ...
  for (int it = 0; it < opt.max_newton; ++it)
  {
    ...
    if (it == 0 && myrank == 0) {
      LOGI("newton", it,
           "stopping rule: ||F|| <= max(atol, rtol*||F0||) with atol={:.3e}, rtol={:.1e}, ||F0||={:.6e}",
           atol, opt.rtol, res0);

      // create/overwrite CSV with header
      if (std::FILE* fph = std::fopen("bratu_newton_trace.csv", "w")) {
        std::fprintf(fph, "it,Fnorm,rel,gmres_it,gmres_rel,linrtol,alpha,step_tmax\n");
        std::fclose(fph);
      }
    }
    ...
    if (abs_ok || rel_ok) {
      ...
      if (myrank == 0) {
        ...
        // Append a CSV row for the convergent step; GMRES/alpha fields are NaN/-1
      ...
      if (myrank == 0) LOGI("time", -1, "solve_time_max={:.6f}s", run_dt_max);
    ...
    if (myrank == 0) {
      ...
      if (std::FILE* fp = std::fopen("bratu_newton_trace.csv", "a")) {
        std::fprintf(fp, "%d,%.16e,%.16e,%d,%.16e,%.1e,%.6f,%.6f\n",
                     it, resnorm, rel, gst.iters, gst.relres, linrtol, alpha, step_dt_max);
        std::fclose(fp);
      }
    }
  }
